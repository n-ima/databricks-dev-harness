...
5. For user-facing work, define an executable mock gate before backend integration.
   ...
   Keep every in-scope screen and its behavior in the design inventory. Plan progressive layout feedback with actual components before full mock acceptance; never reduce the final design to representative screens or require all screens finished before the first discussion. A layout-only request does not authorize backend implementation.
