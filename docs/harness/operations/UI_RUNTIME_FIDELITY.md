...
状態: 2026-09-15 ローカル改善候補。正式採用・リリース・既存案件への更新は別判断。
...
5. 下記のUI対応契約をagentが作り、採用版の部品API/制約と各画面/状態を別contextでレビューする。tree/table等の実現方法や差異は未記録のままにしない。利用者には日本語で画面と差分を見せる。
...
このローカル候補を作っただけでは、利用中案件へ届かない。正式リリース後、[安全な更新入口](SAFE_LOCAL_UPDATE.md)で案件を更新し、新contextで読み直す。
