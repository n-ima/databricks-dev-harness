...
provider向け生成済みファイルも配布物に含まれるため、更新後に`agent-assets:sync`を無条件実行する必要はない。是正後の同期は未知のskillやリンクを保持して停止し、管理fileの上書き前には専用backupを作る。旧版の同期は生成先の再作成で独自skillが消える恐れがあるため、更新前には実行しない。checkで独自ファイルによる不一致が出ても自動修復せず、保持したまま差分をレビューする。
