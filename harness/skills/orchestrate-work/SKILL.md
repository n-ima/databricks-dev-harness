...
- In the harness source (no product.config.json), a main/push/distribution request uses `publish-harness`, not the Databricks `release-work` skill. Include packaging/stamp/update-path verification in publication; advice remains read-only. Source maintenance uses `docs/harness/operations/HARNESS_DEVELOPMENT.md` and its guard setup, never product initialization.
- In an initialized product, a request only to commit/push that product is an existing-work Git operation, not a new product-intent/intake or harness publication. Inspect the existing session, diff, target branch/remote, reviews and tests; preserve unrelated changes and perform only the authorized ordinary Git operation. A fallback `define` hint does not require inventing new requirements. Product Git push does not authorize Databricks deployment, harness stamping or installing the source-only hook. An explicit status-only request remains read-only in both repository types, regardless of route keywords.
