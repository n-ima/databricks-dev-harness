// FIX-02/03 regression cases copied from the 2026-09-16 independent review.
// Only fresh temp fixtures and local Node children; no real providers or DB.
...
async function fixture(t){
  const base=resolve(tmpdir()),root=await mkdtemp(join(base,'truth-gates-regression-'));
  t.mock.method(console,'log',()=>{});
  t.after(async()=>{assert.ok(root.startsWith(base+sep)&&root.slice(base.length+1).startsWith('truth-gates-regression-'));await rm(root,{recursive:true,force:true,maxRetries:5,retryDelay:20});});
...
test('IND-17 async runner bounds immediate child runtime',async t=>{
  const c=await fixture(t);const start=Date.now();const result=await commandResultAsync(process.execPath,['-e','process.stdout.write(String(process.pid));setInterval(()=>{},1000)'],{cwd:c.root,timeout:500});
