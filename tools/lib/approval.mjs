...
import { cleanInline, optionList, pathInside, readJson, timestamp, writeJson, parseFrontmatter, replaceFrontmatterField, atomicWrite, withFileLock, assertNoSecrets } from "./shared.mjs";
...
export async function createApproval(root, options) {
  ...
  if (options.gate === "product-intent") {
    ...
    const requirement = parseFrontmatter(await readFile(pathInside(root, initialSession.requirement), "utf8"));
  ...
  if (options.gate === "product-intent") artifactHashes[initialSession.requirement] = await fileHash(root, initialSession.requirement);
