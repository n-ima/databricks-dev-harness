...
// Advisory candidate, deliberately outside the completion/approval/distribution paths.
...
export async function checkDelivery(root,c,{phase='design'}={}) {
  ...
  const report=()=>({mode:'advisory',certifiesAcceptance:false,phase,findings,
    executed:Array.isArray(c?.testCases)?c.testCases.filter(t=>['pass','fail'].includes(t?.result?.status)).length:0});
