...
import * as updater from '../../tools/lib/distribution.mjs';
...
async function fixture(t, withRunner = false) {
  ...
  const root = await fs.mkdtemp(join(tempParent, 'safe-update-independent-'));
  t.after(async () => {
    ...
    assert.ok(actual.slice(actualParent.length + 1).startsWith('safe-update-independent-'));
