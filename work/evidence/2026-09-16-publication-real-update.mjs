...
assert.equal(current.version,'0.7.0');
const oldRoot=join(root,'.harness/releases/0.6.1');
const oldBytes=await readFile(join(oldRoot,'manifest.json')),old=JSON.parse(oldBytes);
const base=resolve(tmpdir()),temp=await mkdtemp(join(base,'publication-real-update-'));
...
function git(cwd,args){return exec(cwd,'git',['-c','core.autocrlf=true',...args]);}
...
try{
  ...
  for(const [name,selected] of [['from-source',clone],['from-payload',join(root,'.harness/releases/0.7.0')]]){
    ...
    if(name==='from-source'){
      ...
      checks.push(`old 0.6.1 CLI plan exit=${legacy.status}; actual update uses the trusted new standalone CLI`);
    }
    const planned=JSON.parse(exec(target,process.execPath,[cli,'plan','--source',selected,'--target',target]).stdout);
    assert.equal(planned.fromVersion,'0.6.1');assert.equal(planned.toVersion,'0.7.0');assert.equal(planned.canApply,true);
    ...
    checks.push(`${name}: 0.6.1 -> 0.7.0, ${current.managedFiles.length} hashes verified, all ${Object.keys(protectedFiles).length} product/local files preserved, repeat idempotent, publication tests pass`);
  ...
  await verifyOld();checks.push('published 0.6.1 manifest and every payload file unchanged');
  await writeFile(`work/evidence/2026-09-16-publication-real-update-${run}.json`,JSON.stringify({status:'pass',checkedAt:new Date().toISOString(),fromVersion:old.version,toVersion:current.version,manifestSha256:sha256(await readFile('harness/base-release.json')),checks,commands,networkUsed:false,realProjectChanged:false,realRepositoryCommitted:false},null,2)+'\n',{flag:'wx'});
  console.log(checks.join('\n'));
}finally{
  assert.ok(temp.startsWith(base+sep+'publication-real-update-'));await rm(temp,{recursive:true,force:true});
}
