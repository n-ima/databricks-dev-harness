...
const oldVersion=process.argv[3]??'0.7.0';assert.ok(['0.7.0','0.7.1'].includes(oldVersion));
const oldRoot=join(root,'.harness/releases',oldVersion);
const oldBytes=await readFile(join(oldRoot,'manifest.json')),old=JSON.parse(oldBytes);
const base=resolve(tmpdir()),temp=await mkdtemp(join(base,'publish-072-update-'));
...
try{
  ...
  for(const [name,selected] of [['from-source',clone],['from-payload',join(root,'.harness/releases/0.7.2')]]){
    ...
    if(name==='from-source'){
      ...
      checks.push(`old ${oldVersion} CLI plan exit=${legacy.status}; actual update uses the trusted new standalone CLI`);
    }
    const planned=JSON.parse(exec(target,process.execPath,[cli,'plan','--source',selected,'--target',target]).stdout);
    assert.equal(planned.fromVersion,oldVersion);assert.equal(planned.toVersion,'0.7.2');assert.equal(planned.canApply,true);
    ...
    checks.push(`${name}: ${oldVersion} -> 0.7.2, ${current.managedFiles.length} hashes verified, all ${Object.keys(protectedFiles).length} product/local files preserved, repeat idempotent, publication tests pass`);
  ...
  await verifyOld();checks.push(`published ${oldVersion} manifest and every payload file unchanged`);
  await writeFile(`work/evidence/2026-09-16-publish-072-update-${run}.json`,JSON.stringify({status:'pass',checkedAt:new Date().toISOString(),fromVersion:old.version,toVersion:current.version,manifestSha256:sha256(await readFile('harness/base-release.json')),checks,commands,networkUsed:false,realProjectChanged:false,realRepositoryCommitted:false},null,2)+'\n',{flag:'wx'});
  console.log(checks.join('\n'));
}finally{
  assert.ok(temp.startsWith(base+sep+'publish-072-update-'));await rm(temp,{recursive:true,force:true});
}
