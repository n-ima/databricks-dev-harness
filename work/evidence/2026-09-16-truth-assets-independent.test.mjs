...
const noExternal = { run() { throw new Error('External execution prohibited'); } };
async function metric(root, extra = {}) {
  await put(root, 'harness/toolchain.lock.json', '{"appkitTemplateVersion":"v0.69.1"}');
  const plan = await planScaffold(root, { kind: 'metric-view', name: 'sales', source_table: 'dev.test.orders', dimension: ['地域=region'], measure: ['Revenue=SUM(amount)'], ...extra }, noExternal);
  await applyScaffold(root, { plan: 'work/scaffolds/' + plan.id + '.json', yes: true }, noExternal);
  return { yaml: await readFile(join(root, 'src/metrics/sales.metric.yml'), 'utf8'), sql: await readFile(join(root, 'src/metrics/sales.draft.sql'), 'utf8') };
}
test('IND-METRIC embedded YAML parses to the canonical YAML with Japanese and quoting', async t => {
  const root = await fixture(t);
  const { yaml, sql } = await metric(root, { description: '説明: "Revenue" と地域 # metadata', dimension: ['地域: "分類"=region'] });
  assert.equal(sql.split('$$').length, 3); const embedded = sql.split('$$')[1];
  assert.equal(embedded, '\n' + yaml); assert.match(sql, /DRAFT ONLY/);
  const parsed = spawnSync('python', ['-X', 'utf8', '-c', 'import json,sys,yaml; x=json.load(sys.stdin); a=yaml.safe_load(x["canonical"]); b=yaml.safe_load(x["embedded"]); assert isinstance(a,dict) and a==b; print(json.dumps({"equal":True,"parser":yaml.__version__}))'], { input: JSON.stringify({ canonical: yaml, embedded }), encoding: 'utf8', timeout: 10000, env: { ...process.env, PYTHONDONTWRITEBYTECODE: '1' } });
  assert.equal(parsed.status, 0, parsed.stderr); assert.equal(JSON.parse(parsed.stdout).equal, true);
});
for (const extra of [{ description: 'Profit $$ USD' }, { dimension: ['地域 $$ 分類=region'] }, { measure: ['Revenue $$ USD=SUM(amount)'] }]) {
  test('IND-METRIC delimiter in metadata is rejected or safely embedded: ' + JSON.stringify(extra), async t => {
    const root = await fixture(t); let output;
    try { output = await metric(root, extra); }
    catch (error) { assert.match(error.message, /delimiter|dollar|\$\$|SQL|metric/i); return; }
    assert.equal(output.sql.split('$$').length, 3, 'A metadata delimiter prematurely closes the SQL string.');
    assert.equal(output.sql.split('$$')[1], '\n' + output.yaml);
    const parsed = spawnSync('python', ['-X', 'utf8', '-c', 'import json,sys,yaml; print(json.dumps(yaml.safe_load(sys.stdin.read())))'], { input: output.yaml, encoding: 'utf8', timeout: 10000, env: { ...process.env, PYTHONDONTWRITEBYTECODE: '1' } });
    assert.equal(parsed.status, 0, parsed.stderr); const values = JSON.parse(parsed.stdout);
    if (extra.description) assert.equal(values.comment, extra.description);
    if (extra.dimension) assert.equal(values.dimensions[0].name, extra.dimension[0].split('=')[0]);
    if (extra.measure) assert.equal(values.measures[0].name, extra.measure[0].split('=')[0]);
  });
}
