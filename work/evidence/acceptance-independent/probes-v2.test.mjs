...
// v2 changes only the obsolete second-read reachability assumption below.
// A single verified read must still reject missing AC-D01 and preserve state.
test('receipt validation must parse the same requirement bytes that it checked against the artifact hash', async t => {
  ...
  await interceptedRead(async () => {
    try { await closeSession(root, { id: 'S-1', summary: 'Must refuse changed snapshot', verifier_evidence: receiptPath }); }
      ...
      rejected = true;
      assert.match(error.message, /Missing acceptance.*AC-D01|Verified artifact changed/);
  ...
  assert.ok(requirementReads >= 1, 'The requirement must actually be read');
  ...
  t.diagnostic(JSON.stringify({ rejected, currentHashMatches, fixtureSessionStatus: status, requirementReads, secondReadMutationReached: changed }));
  assert.equal(rejected, true, 'Incomplete receipt must never complete after a snapshot change.');
  assert.equal(status, 'active');
  assert.deepEqual(await readFile(join(root, sessionPath)), before);
});

for (const changedPath of [requirement, review]) {
  test('seal rejects a changed parsed input and preserves an existing output: ' + changedPath, async t => {
    const root = await fixture(t);
    const reviewRecord = {
      reviewer: 'independent-probe-reviewer', provider: 'copilot', independent: true,
      acceptance: ['AC-01', 'AC-D01'].map(id => ({ id, status: 'pass', evidence: [evidence] })),
    };
    await writeJson(join(root, review), reviewRecord);
    await atomicWrite(join(root, receiptPath), 'Existing synthetic output must not be overwritten.\n');
    const preservedPaths = [receiptPath, sessionPath];
    const before = await bytes(root, preservedPaths);
    let changed = false;
    await interceptedRead(async () => {
      await assert.rejects(sealEvidence(root, { review, requirement, session: 'S-1', output: receiptPath }), /Evidence input changed after validation/);
    }, async path => {
      if (!changed && resolve(String(path)) === resolve(root, evidence)) {
        changed = true;
        if (changedPath === requirement) await atomicWrite(join(root, requirement), document('- AC-01: modified after parsing'));
        else await writeJson(join(root, review), { ...reviewRecord, acceptance: reviewRecord.acceptance.slice(0, 1) });
      }
    });
    assert.equal(changed, true, 'Both inputs must have been parsed before the evidence boundary was reached');
    await unchanged(root, preservedPaths, before);
    t.diagnostic(JSON.stringify({ changedPath, changed, existingOutputPreserved: true, sessionPreserved: true }));
  });
}

test('receipt rejects a changed requirement at its first actual read and preserves the session', async t => {
  const root = await fixture(t);
  await directReceipt(root, ['AC-01']);
  const before = await readFile(join(root, sessionPath));
  let reads = 0, changed = false;
  await interceptedRead(async () => {
    await assert.rejects(closeSession(root, { id: 'S-1', summary: 'Reject pre-read mutation', verifier_evidence: receiptPath }), /Verified artifact changed/);
  }, async path => {
    if (resolve(String(path)) === resolve(root, requirement) && ++reads === 1) {
      changed = true;
      await atomicWrite(join(root, requirement), document('- AC-01: changed before hash validation'));
    }
  });
...
test('unchanged valid seal and receipt bind the actual review and requirement bytes', async t => {
  const root = await fixture(t);
  await writeJson(join(root, review), {
    reviewer: 'independent-probe-reviewer', provider: 'copilot', independent: true,
    acceptance: ['AC-01', 'AC-D01'].map(id => ({ id, status: 'pass', evidence: [evidence] })),
  });
  const sealed = await sealEvidence(root, { review, requirement, session: 'S-1', output: receiptPath });
  assert.equal(sealed.status, 'pass');
  assert.equal(sealed.artifactHashes[review], await fileHash(root, review));
  assert.equal(sealed.artifactHashes[requirement], await fileHash(root, requirement));
  const validated = await validateReceipt(root, receiptPath, { sessionId: 'S-1', requirement, implementer: 'claude' });
  assert.deepEqual(validated.acceptance.map(item => item.id), ['AC-01', 'AC-D01']);
  assert.equal(parseFrontmatter(await readFile(join(root, sessionPath), 'utf8')).status, 'active');
});

test('new generated-question exception is limited to the expected ledger depth and shape', () => {
  const base = '- AC-01: visible\n';
  const ledger = '## Refined answers — authoritative question ledger\n';
  assert.deepEqual(acceptanceIds(base + ledger + '### Q-04 acceptance\nQuestion and answer.\n'), ['AC-01']);
  for (const tail of [
    '### Q-04 acceptance\n',
    ledger + '#### Q-04 acceptance\n',
    ledger + '### Q-04: acceptance\n',
    ledger + '### Q-04 Acceptance\n',
    ledger + '## Notes\n### Q-04 acceptance\n',
    '\u200b• AC-D01: hidden prefix\n',
  ]) assert.throws(() => acceptanceIds(base + tail), /Invalid acceptance/);
});
